...
func SetCookieStore(sc *SessionConfig) func(*OAuthProxy) error {
	return func(op *OAuthProxy) error {
		cookieStore, err := sessions.NewCookieStore(sc.CookieConfig.Name,
			sessions.CreateMiscreantCookieCipher(sc.CookieConfig.decodedSecret),
			func(c *sessions.CookieStore) error {
				c.CookieDomain = sc.CookieConfig.Domain
				c.CookieHTTPOnly = sc.CookieConfig.HTTPOnly
				c.CookieExpire = sc.CookieConfig.Expire
				c.CookieSecure = sc.CookieConfig.Secure
				return nil
			})
...
func SetUpstreamConfigs(uc *UpstreamConfigs, sc SessionConfig, svc ServerConfig) error {
	if uc.ConfigsFile != "" {
...
func newProvider(cc ClientConfig, pc ProviderConfig, sc SessionConfig, statsdClient *statsd.Client) (providers.Provider, error) {
	providerURL, err := url.Parse(pc.ProviderURLExternal)
	if err != nil {
		return nil, err
	}
